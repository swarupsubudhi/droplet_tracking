...
class FrameReader:
    ...
    def get_frame(self):
        with self.lock:
            return (None, self.pos) if self.frame is None else (self.frame.copy(), self.pos)
...
# ----------------------------
# Detection using per-view parameters, returning stages
# ----------------------------
def detect_circle_with_params_from_sliders(crop, params):
    ...
    mask = cv2.inRange(hsv, lower, upper)  # binary mask (0/255)
    ...
    gray = (crop[..., 0] * b_n + crop[..., 1] * g_n + crop[..., 2] * r_n).astype(np.uint8)

    # apply mask to gray so blur focuses on thresholded area, but show pure binary as requested
    gray_masked = cv2.bitwise_and(gray, gray, mask=mask)

    # blurred stage (on gray_masked)
    blur = cv2.GaussianBlur(gray_masked, (9, 9), 2)

    # circle detection on blurred
    ...
    if circles is not None and len(circles) > 0:
        ...
        cv2.circle(processed, (int(x), int(y)), int(r), (0, 255, 0), 2)
        circ = 2 * np.pi * r
        cv2.putText(processed, f"C={circ:.1f}", (int(x + r + 5), int(y)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)

    # return raw, binary mask, blurred gray, processed
    return (x, y, r), [raw, mask, blur, processed]

# ----------------------------
# Main UI with threaded reader and 4-stage previews per view
# ----------------------------
class DropletTrackerUI:
    def __init__(self, root, video_path, start_frame=0):
        self.root = root
        self.root.title("Droplet Tracker v0.3 — Diagnostic (threaded + stages)")
        ...
        # Stage preview labels (4 per view): raw, mask, blur, processed
        self.front_stage_labels = [tk.Label(self.front_frame) for _ in range(4)]
        for lbl in self.front_stage_labels:
            lbl.pack(side=tk.LEFT, padx=4)

        self.side_stage_labels = [tk.Label(self.side_frame) for _ in range(4)]
        for lbl in self.side_stage_labels:
            lbl.pack(side=tk.LEFT, padx=4)
    ...
    def _build_param_sliders_grid(self, parent, view="front"):
        ...
        def add_slider_grid(label_text, default, rng):
            ...
            s = tk.Scale(inner, from_=rng[0], to=rng[1], orient=tk.HORIZONTAL, length=300, resolution=0.01 if isinstance(default, float) else 1)
            s.set(default)
    ...
    def log_resume_params(self):
        ...
        self.log(
            ...
            f"Hough(dp={fp['dp']},minD={fp['minDist']},p1={fp['param1']},p2={fp['param2']},"
            f"r=[{fp['minRadius']}-{fp['maxRadius']}]), "
            f"Gray(R={fp['r_w']:.2f},G={fp['g_w']:.2f},B={fp['b_w']:.2f})\n"
            f"  Side:  H={sp['h_range']}, S={sp['s_range']}, V={sp['v_range']}, "
            f"Hough(dp={sp['dp']},minD={sp['minDist']},p1={sp['param1']},p2={sp['param2']},"
            f"r=[{sp['minRadius']}-{sp['maxRadius']}]), "
    ...
    def _process_loop_once(self):
        ...
        # Resize and crop views
        frm_resized = cv2.resize(frm, (1920, 540))
        side_crop = frm_resized[:, 240:240+480]
        front_crop = frm_resized[:, 1200:1200+480]

        self._process_and_display_view("front", front_crop, self.front_stage_labels, self.front_metrics)
        self._process_and_display_view("side", side_crop, self.side_stage_labels, self.side_metrics)
    ...
    def _ui_update_loop(self):
        if not self.running:
            ...
            if frm is not None:
                ...
                side_crop = frm_resized[:, 240:240+480]
                front_crop = frm_resized[:, 1200:1200+480]
                self._process_and_display_view("front", front_crop, self.front_stage_labels, None)
                self._process_and_display_view("side", side_crop, self.side_stage_labels, None)
    ...
    # Shared view processing and stage preview update
    def _process_and_display_view(self, view_name, crop, stage_labels, metrics_list):
        params = self.get_params(view_name)
        (x, y, r), stages = detect_circle_with_params_from_sliders(crop, params)
        ...
        # Update stage thumbnails (scale down by 2x)
        for lbl, stage in zip(stage_labels, stages):
            if stage.ndim == 2:  # grayscale or binary mask
                stage = cv2.cvtColor(stage, cv2.COLOR_GRAY2RGB)
            h, w = stage.shape[:2]
            new_size = (max(1, w // 2), max(1, h // 2))
            disp = cv2.resize(stage, new_size)
            ...
            if success:
                ...
                lbl.image = img  # keep reference to avoid GC
