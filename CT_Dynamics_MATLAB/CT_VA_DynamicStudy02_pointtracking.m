...
% Preallocate arrays
N = 239; % adjust to number of frames processed
...
% Parameters
delt   = 5/24;         % seconds between processed frames
px2mm  = 113;          % pixels per mm for 600x2160 px image
resize = 0.2;          % resize factor
...
if ~isempty(jpgfiles)
    ...
    given_pt = [338, -518]; % (x,y) with y negative meaning "upward"
    ...
    % Initialize tracker
    tracker1 = vision.PointTracker('MaxBidirectionalError', 2.0);
...
% -------------------------------
tracker2 = vision.PointTracker('NumPyramidLevels',4,'MaxBidirectionalError', 4.0,...
    'BlockSize',[15 15]);
...
for k = 1:5:length(jpgfiles)
    ...
    [center, radius] = imfindcircles(gray_img,[40 170], ...
    ...
    line(point1_traj(1:i,1),point1_traj(1:i,2),'Color',[0 0.6 1 0.5],'LineWidth',1.5);

    % --- Point 2 tracking (initialize at frame 50) ---
    if ~point2_initialized && i == 188
        % Define initial coordinates for point 2 at t=50
        x2 = 285*resize; % example pixel coordinate
        y2 = 1350*resize; % example pixel coordinate
    ...
    elseif point2_initialized
        ...
        line(point2_traj(1:i,1),point2_traj(1:i,2),'Color',[0.2 0.8 0.2 0.5],'LineWidth',1.5);
    end
...
plot(realtime,zcoord_1,'r--')
plot(realtime,xcoord_0,'b--')
plot(realtime,xcoord_2,'r-.')
plot(realtime,xcoord_0,'b-.')
xlim([0 te])
%ylim([-20 5])
xlabel('time')
...
plot(realtime,velz_p1_smooth,'r--')
plot(realtime,velx_p1_smooth,'b--')
plot(realtime,velz_p2_smooth,'r-.')
